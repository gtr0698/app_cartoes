import 'package:flutter_application_aula3_cadastro/Data/conexao.dart';
import 'package:flutter_application_aula3_cadastro/Data/perfil_Entity.dart';
...
class senhaSQLiteDatasource{
  Future inserirSenha(descricao, email, senha) async {
    final db = await Conexao.getConexaoDB();
    senha.senhaID = await db.rawInsert('''insert into
      $SENHA_TABLE_NAME(
  ...
  Future<SenhaEntity> getSenhaId(senhaID) async {
    ...
    senha.descricao = dbItem['login'] as String?;
    senha.email = (dbItem['email'] as String?)!;
  ...
  Future<void> deletarSenha(senhaId) async {
    final db = await Conexao.getConexaoDB();
    await db.transaction((txn) async {
      await txn
